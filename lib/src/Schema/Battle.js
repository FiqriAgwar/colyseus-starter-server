...
class BattleSchema extends schema_1.Schema {
    constructor(nCollectibles) {
        ...
        this.collectibles = new schema_1.MapSchema();
        this.collectibles = new schema_1.MapSchema();
        for (let i = 0; i < nCollectibles; i++) {
            this.collectibles.set(i.toString(), new Collectible_1.default(Math.floor(Math.random() * Math.pow(2, 12) + 1), Math.floor(Math.random() * Math.pow(2, 12) + 1), i));
        }
    }
}
__decorate([
...
__decorate([
    (0, schema_1.type)({ map: Collectible_1.default })
