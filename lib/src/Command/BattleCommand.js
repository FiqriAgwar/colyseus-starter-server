...
exports.OnPlayerCollecting = exports.OnPlayerMove = exports.OnPlayerSpawn = exports.OnPlayerLeave = exports.OnPlayerJoin = void 0;
...
class OnPlayerCollecting extends command_1.Command {
    execute({ sessionId, coinId }) {
        ...
        player.point += 1;
        var nX = Math.floor(Math.random() * Math.pow(2, 12)) + 1;
        var nY = Math.floor(Math.random() * Math.pow(2, 12)) + 1;
        coin.position.assign({
            x: nX,
            y: nY,
        });
        this.room.broadcast("collected", { coinId, nX, nY });
    }
}
